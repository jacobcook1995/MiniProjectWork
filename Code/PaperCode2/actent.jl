...
function plotting()
    ...
    p3 = plot(dpi=300,title="Toggle Switch Production/Flow Terms",xlabel="Concentration a")
        ...
        for j = 1:2
            ...
            if isfile(infile)
                ...
                if j == 1
                    ...
                    plot!(p2,path2[1:end-1,1],PE[:,j]*mag1,label=L"PE",color=2)
                    plot!(p2,path2[1:end-1,1],KE[:,j]*mag1,label=L"KE",color=3)
                    plot!(p3,path2[1:end-1,1],prods*mag1,label=L"EP",color=1)
                    plot!(p3,path2[1:end-1,1],flows*mag1,label=L"EF",color=2)
                    plot!(p3,path2[1:end-1,1],(prods.-flows)*mag1,label=L"EP-EF",color=3)
                else
                    ...
                    plot!(p3,path2[1:end-1,1],prods*mag1,label="",color=1,style=:dash)
                    plot!(p3,path2[1:end-1,1],flows*mag1,label="",color=2,style=:dash)
                    plot!(p3,path2[1:end-1,1],(prods.-flows)*mag1,label="",color=3,style=:dash)
                end
            else # Tell users about missing files
                if j == 1
                    println("No A to B path for $(i)")
    ...
    plot!(p2,path2[1:end-1,1],(Af[:,2]-Af[end:-1:1,1])*mag1,label=L"\mathcal{A}_{B\rightarrow A} - \mathcal{A}_{A\rightarrow B}",color=4,style=:dot)
    plot!(p2,path2[1:end-1,1],(ΔSf[end:-1:1,1]-ΔSf[:,2])*mag1,label=L"\Delta S_{A\rightarrow B} - \Delta S_{B\rightarrow A}",color=5,style=:dot)
