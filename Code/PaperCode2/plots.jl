...
function main()
    ...
    # Now open affinity and flux data
    # Open flux data
    infile = "../Results/Fig3Data/$(ARGS[1])fluxes.csv"
    l = countlines(infile)
    w = 8
    fluxes = zeros(l,w)
    open(infile, "r") do in_file
        ...
        for line in eachline(in_file)
            ...
            comma[end] = L+1
            for i = 1:w
                fluxes[k,i] = parse(Float64,line[(comma[i]+1):(comma[i+1]-1)])
            end
            k += 1
        end
    end
    # Then open affinity data
    infile = "../Results/Fig3Data/$(ARGS[1])affs.csv"
    l = countlines(infile)
    w = 8
    affs = zeros(l,w)
    open(infile, "r") do in_file
        ...
        for line in eachline(in_file)
            ...
            comma[end] = L+1
            for i = 1:w
                affs[k,i] = parse(Float64,line[(comma[i]+1):(comma[i+1]-1)])
            end
            k += 1
        end
    end
    scatter([ent[:,3].-ent[:,4]],[fluxes[:,1]-fluxes[:,8]],label="")
    plot!(xlabel=L"\dot{S}_{h}-\dot{S}_{l}",ylabel=L"\mathcal{F}_{h}-\mathcal{F}_{l}")
    plot!(title=L"\Delta\mathcal{F}\;vs\;\Delta\dot{S}",top_margin=8.0mm)
    savefig("../Results/EntProdvsFluxDiff.png")
    return(nothing)
end
