...
function timdis(ts::Array{Float64,1},x::Array{Float64,1},NG::Int64,NM::Int64,Tmid::Float64)
...
function act(x::Array{Float64,1},Tp::Float64,b::Array{SymEngine.Basic,1},Dmin::Array{SymEngine.Basic,2},ps::Array{Float64,1},Ns::Int64)
    ...
    return(Ac,ΔS,Acf,ΔSf,KE,PE,prods,flows,fs,qs,ΔS1,ΔS2)
end

# calculate +ve step γ+
function γpos(X::Float64,ps::Array{Float64,1})
    y = ps[1] + ps[4]*X^2
    return(y)
end

# calculate -ve step γ-
function γneg(X::Float64,ps::Array{Float64,1})
    y = ps[2]*X + ps[3]*X^3
    return(y)
end

# find difference from actual rate as would be expected from the master equation
function Δ(x::Array{Float64,1},Ns::Int64,ps::Array{Float64,1})
    V = zeros(Ns)
    for i = 1:length(x)
        γ1 = γpos(x[i],ps)
        γ2 = γneg(x[i],ps)
    ...
    for i = 1:l
        for j = 1:2
            ...
            if isfile(infile)
                ...
                Tmid = tims2[Nmid]
                Tp = tims2[end]
                # Ns now found using the time of the saddle point
                ...
                Act, ΔS, _, _, _, _, prods, flows, _, _, ΔS1, ΔS2 = act(path2,Tp,b,Dmin,ps[i,:],Ns)
                ...
                line = "$(sum(prods)),$(sum(flows)),$(ΔS1),$(ΔS2)\n"
        ...
        for j = 1:2
            ...
            if isfile(infile)
                ...
                if j == 1
                    ...
                    plot!(p2,path2[1:end-1],ΔSf*mag1,label=L"\Delta S^L",color=8)
                    ...
                    plot!(p2,path2[1:end-1],ΔSf*mag1,label="",color=8,style=:dash)
