#!/usr/bin/env julia
# schswitch.jl
# A script to make nice plots of a one species schlögl simulation showing switching
...
function main()
    ...
    pyplot()
    plot(ts,Xs,xlabel="Time t",ylabel="Copy number X",title="1D Schlögl",titlefontsize=20,label="",dpi=300,guidefontsize=16,legendfontsize=14)
    hline!([star1],color=:black,linestyle=:dash,label="low state",tickfontsize=13)
