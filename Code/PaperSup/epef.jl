...
function second()
    ...
    # Need to calculate both EP and EF terms along path
    # Therefore need to read in parameter sets
    # First read in parameter sets
    infile = "../Results/Fig3Data/$(ARGS[1])para.csv"
    if ~isfile(infile)
        println("Error: No file of parameters to be read for toggle.")
        return(nothing)
    end
    ...
    infile = "../Results/Fig3DataS/$(ARGS[2])paraS.csv"
    if ~isfile(infile)
        println("Error: No file of parameters to be read for Schlögl.")
        return(nothing)
    end
    # now read in parameters
    l = countlines(infile)
    ...
    open(infile, "r") do in_file
        ...
        for line in eachline(in_file)
            ...
            comma[end] = L+1
            for i = 1:w
                psS[k,i] = parse(Float64,line[(comma[i]+1):(comma[i+1]-1)])
            end
            k += 1
        end
    end
    # Now need to read in trajectories
    return(nothing)
end
