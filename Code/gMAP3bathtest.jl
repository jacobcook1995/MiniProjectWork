...
function discretise(x::AbstractArray,NG::Int)
    ...
    for i = 2:NG+1
        ...
        dB = x[i,2] - x[i-1,2]
        dS = x[i,3] - x[i-1,3]
        s[i] = s[i-1] + sqrt(dA^2 + dB^2 + dS^2) # Could probably drop the sqrts to speed up the code
    end
...
function linsys(x::AbstractArray,xprim::AbstractArray,λs::AbstractVector,ϑs::AbstractArray,λprim::AbstractVector,
                ...
                Hθx::Array{SymEngine.Basic,2},Δτ::Number,NG::Int,H::SymEngine.Basic)
...
function gMAP(K,k,Q,q,kmin,qmin,f,r,F,NM::Int,NG::Int,Δτ,high2low::Bool)
...
function main()
    ...
    path = gMAP(K,k,Q,q,kmin,qmin,f,r,F,NM,NG,Δτ,high2low)
    plot(path[:,1],path[:,2])
