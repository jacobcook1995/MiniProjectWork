...
function graphs2()
    ...
    plot(segcent1,2*N1*(ents1[:,1]+ents1[:,2])/t1,title="$(2*sum(ents1))",label="Entropy Production",xlabel="A",ylabel="\\Delta S")
    ...
    plot(segcent2,2*N2*(ents2[:,1]+ents2[:,2])/t2,title="$(2*sum(ents2))",label="Entropy Production",xlabel="A",ylabel="\\Delta S")
